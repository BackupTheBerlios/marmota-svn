...
public class ConnectionHandler extends Thread {
		...
		/**
		 * The size of the buffer which will be used for
		 * sending data to the client before a flush
		 */
         ...
         * @param outputStream The OutputStream, get from socket
         * @param requestedPage The requested Page
         * @param host The Host (send by the client with the request)
         */
        private void sendClientData(OutputStream os,String requestedPage, String host) {
        	Marmota.getLogger().debug("Conhandler: " + handlerid + ": sendClientData requested Page: " + requestedPage);
        		...
        		try {
            		...
            		for (int i = 0; i < pluginnames.size(); i++) {
            			if (pluginnames.get(i).equals(page)) {
            				...
            				Marmota.getLogger().debug("Conhandler: " + handlerid + ": requested page has " + localfile.length() + "bytes");
        ...
        private void sendjnlpdescriptor(OutputStream os, String host) {
                try {
                        ...
                        for (int i = 0; i < pluginnames.size(); i++) {
                                sendBuffer.append("<jar href=\"" + pluginnames.get(i) + "\" download=\"eager\"/>\n");
                        }
                        sendBuffer.append("</resources>\n");
        ...
        /**
         * Should be used to create a http-header for the data which should be send
         * @param os The Ouputstream
         * @param url The url of the file which should been send
         * @throws IOException 
         */
        private void sendHeader(OutputStream os, URL url) throws IOException {
        	sendHeader(os, url, null);
        }
         ...
         * @throws IOException
         */
        private void sendHeader(OutputStream os, URL url, ArrayList<String> optionalHeaderLines) throws IOException {
                // Major Head-Informations
                StringBuffer headbuffer = new StringBuffer();
                // headbuffer.append("HTTP/1.0 200 OK\r\n");
                headbuffer.append("HTTP/1.1 302 Found\r\n");
                headbuffer.append("Server: " + Marmota.PNAME + " WebStart-WebServer " + 
                                Marmota.MAJORVERSION + "." + Marmota.MINORVERSION + " " + 
                                Marmota.VERSIONSUFFIX + " on " + 
                                System.getProperty("os.name") + " " +
                                System.getProperty("os.version") + " " +
                                System.getProperty("os.arch") + "\r\n");
                Date date = new Date();
                headbuffer.append("Date: " + new SimpleDateFormat("EEE, d MMM yyyy HH:mm:ss Z", new Locale("US")).format(date) + "\r\n");
                headbuffer.append("Last-Modified: " + new SimpleDateFormat("EEE, d MMM yyyy HH:mm:ss Z", new Locale("US")).format(date) + "\r\n");
                headbuffer.append("Cache-Control: no-cache\r\n");
                headbuffer.append("Pragma: no-cache\r\n");
                ...
                // Adding optional HeaderLines
                if (optionalHeaderLines != null) {
                	for (String str : optionalHeaderLines) {
                		headbuffer.append(str + "\r\n");
                	}
                }
                // Logging the header, if debugging is active
                for (String str : headbuffer.toString().split("\n")) {
                	Marmota.getLogger().debug("Connection handler " + handlerid + ": sending Header: " + str);
                }
        ...
        */
        private String httpError(int code, String description) {
                StringBuffer buffer = new StringBuffer();
                Marmota.getLogger().debug("Handler " + handlerid + ": sending errorpage with: " + code + ", " + description);
